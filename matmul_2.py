...
# this kernel deadlocks, cant figure out why, probably something to do with transaction count
class BarrierArray:
    ...
    def init_barriers(self):
        ...
        def then_body():
            for index in cutlass.range(self.num_stages):
    ...
    def get_barrier(self, index: int) -> cute.Pointer:
        return self.barrier_storage + index
    
    def arrive(self, index: int):
        cute.arch.mbarrier_arrive(self.get_barrier(index))
    
    # this is broken, arriving at the barrier will flip the phase bit
    # arrive and wait need to be part of the same op
    def arrive_and_wait(self, index: int, phase: int):
        cute.arch.mbarrier_arrive(self.get_barrier(index))
        cute.arch.mbarrier_wait(self.get_barrier(index), phase)
    
    # This seems to not work
    # what we would want to use here is the mbarrier_arrive_and_expect_tx_bytes
    # https://docs.nvidia.com/cutlass/media/docs/pythonDSL/cute_dsl_api/cute_arch.html#cutlass.cute.arch.mbarrier_arrive_and_expect_tx
    # but it is not yet implemented in the cutlass dsl
    def arrive_and_expect_tx_bytes(self, index: int, tx_count: int):
        with cute.arch.elect_one():
            cute.arch.mbarrier_init_tx_bytes(self.get_barrier(index), tx_count)
    
    def __get_mlir_types__(self):
        return [self.num_stages.type, self.arrive_count.type]
    
    def __extract_mlir_values__(self):
        return [self.num_stages.ir_value(), self.arrive_count.ir_value()]
    
    def __new_from_mlir_values__(self, values):
        return BarrierArray(
            barrier_storage=self.barrier_storage,
            num_stages=Int32(values[0]),
            arrive_count=Int32(values[1])
        )
...
class GemmKernel:
    ...
    def kernel(self,
        ...
        mainloop_pipeline_array_ptr = storage.mainloop_pipeline_array_ptr.data_ptr().align(min_align=8)
        ...
        full_array = BarrierArray(
            barrier_storage=mainloop_pipeline_array_ptr,
            num_stages=Int32(self.PIPELINE_STAGES),
            # arrive_count=Int32(1)
            arrive_count=Int32(1 + self.num_consumer_warpgroups * 128)
            # arrival count is 1 thread from producer warpgroup, and all threads from consumer warpgroups
        )

        empty_array = BarrierArray(
            barrier_storage=mainloop_pipeline_array_ptr + self.PIPELINE_STAGES, # alignment of 8 is correct here?
            num_stages=Int32(self.PIPELINE_STAGES),
            # arrive_count=Int32(1)
            arrive_count=Int32(1 + self.num_consumer_warpgroups * 128)
            # arrival count is 1 thread from producer warpgroup, and all threads from consumer warpgroups
        ...
        # barrier_arrive_count = self.num_producer_warpgroups + self.num_consumer_warpgroups * 128
        # if warp_idx == 0:
        #     for i in range(self.PIPELINE_STAGES):
        #         cute.arch.mbarrier_init_arrive_cnt(full_array_ptr + i, barrier_arrive_count)
        #         cute.arch.mbarrier_init_arrive_cnt(empty_array_ptr + i, barrier_arrive_count)
        # full_array = [full_array_ptr + i for i in range(self.PIPELINE_STAGES)]
        # empty_array = [empty_array_ptr + i for i in range(self.PIPELINE_STAGES)]
            ...
            if tidx == 256:
                ...
                while work_tile.is_valid_tile:
                    ...
                    for tile_k in cutlass.range_dynamic(num_tiles_k):
                        ...
                        if bidz == 0 and (tidx == 0 or tidx == 256):
                            cute.printf("producer tile_k: {}, index: {}", tile_k, producer_state.index)
                        
                        # producer wait for buffer to be empty
                        empty_array.arrive_and_wait(producer_state.index, producer_state.phase)
                        
                        # producer arrives at full barrier, and sets its tx count to tma_copy_bytes
                        # threads that wait at this barrier will wait until tma_copy_bytes bytes have been written to the barrier
                        full_array.arrive_and_expect_tx_bytes(producer_state.index, tma_copy_bytes)
                        # full_array.arrive(producer_state.index)

                        # if bidz == 0:
                        #     cute.printf("{} arrived at full barrier {}", tidx, producer_state.index)
                        ...
                        cute.copy(
                            ...
                            tAgA_k,
                            tAsA_stage,
                            tma_bar_ptr=full_array.get_barrier(producer_state.index),
                            mcast_mask=0
                        )
                        cute.copy(
                            tma_atom_b,
        ...
        if is_consumer:
            ...
            while work_tile.is_valid_tile:
                ...
                for tile_k in cutlass.range_dynamic(num_tiles_k):
                    ...
                    if bidz == 0 and (tidx == 0 or tidx == 256):
                        cute.printf("consumer tile_k: {}, index: {}", tile_k, consumer_state.index)
                    
                    # if bidz == 0:
                    #     cute.printf("{} arrived at full barrier {}", tidx, consumer_state.index)
                    
                    # consumer waits for bytes to arrive at full barrier
                    full_array.arrive_and_wait(consumer_state.index, consumer_state.phase ^ 1)
                    ...
                    # consumer arrives at empty barrier to signal that it is done with the tile, and producer can write
                    empty_array.arrive(consumer_state.index)
